...
# AWS CLI is used only to write ~/.aws/config — install it if missing
if ! command -v aws &> /dev/null; then
    ...
    if ! command -v aws &> /dev/null; then
        echo "WARNING: AWS CLI installation failed. You may need to install it manually: https://aws.amazon.com/cli/"
        echo "         Continuing anyway — AWS CLI is only needed to write ~/.aws/config."
    else
