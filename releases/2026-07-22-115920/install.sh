...
# ---------------------------------------------------------------------------
RELEASE_BASE_URL="https://raw.githubusercontent.com/theiconic/ClaudeCodeSetup/main/releases/2026-07-22-115920"
...
if [ -f ~/claude-code-with-bedrock/quota-poller ]; then
    QP_VERSION=$(~/claude-code-with-bedrock/quota-poller --version 2>&1 || true)
    echo "  OK quota-poller: $QP_VERSION"
    MODELS_OUTPUT=$(~/claude-code-with-bedrock/quota-poller --models --statusline 2>/dev/null || true)
    if echo "$MODELS_OUTPUT" | grep -q '"models"'; then
        ...
        echo "  WARN per-model query: unexpected output from --models --statusline"
    fi
else
