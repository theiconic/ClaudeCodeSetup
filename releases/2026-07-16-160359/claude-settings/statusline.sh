...
main() {
    ...
    if [ "$cost_enabled" = "true" ] && [ -x "$poller" ] && [ -n "$active_profile" ]; then
        ...
        if [ -f "$cost_cache" ]; then
            local cost_time
            cost_time=$(stat -f %m "$cost_cache" 2>/dev/null || stat -c %Y "$cost_cache" 2>/dev/null || echo 0)
            cost_age=$(( $(date +%s) - cost_time ))
        fi
        if [ "$cost_age" -gt "$cost_cache_ttl" ]; then
            ("$poller" --profile "$active_profile" --statusline --include-cost > "$cost_cache.tmp" 2>/dev/null && mv "$cost_cache.tmp" "$cost_cache") &
        fi
    fi
    ...
    if [ -n "$quota_cache" ] && [ -f "$quota_cache" ]; then
        ...
        if [ "$cost_enabled" = "true" ] && [ -f "$cost_cache" ]; then
            ...
            if [ -n "$today_usd" ] && [ -n "$month_usd" ]; then
                # Round to 2 decimal places for display
                today_disp=$(printf '%.2f' "$today_usd" 2>/dev/null || echo "$today_usd")
                month_disp=$(printf '%.2f' "$month_usd" 2>/dev/null || echo "$month_usd")
                line2+="  ${DM}|${D} 💰 ${G}\$${today_disp}${DM}/day  \$${month_disp}${DM}/mo${D}"
            fi
        fi

        # Alert message — third line when quota is warned or blocked
        local alert_level alert_message
        alert_level=$(grep -o '"alert_level"[[:space:]]*:[[:space:]]*"[^"]*"' "$quota_cache" | sed 's/.*:.*"\([^"]*\)"/\1/' | head -1)
        alert_message=$(grep -o '"alert_message"[[:space:]]*:[[:space:]]*"[^"]*"' "$quota_cache" | sed 's/.*:.*"\([^"]*\)"/\1/' | head -1)

        if [ -n "$alert_message" ]; then
            local alert_color
            if [ "$alert_level" = "blocked" ]; then
                alert_color="\033[1;31m"  # bold red
            else
                alert_color="\033[1;33m"  # bold yellow
            fi
            echo -e "$line1"
            echo -e "$line2"
            echo -e "${alert_color}⚠  ${alert_message}\033[0m"
            return
        fi
    fi
